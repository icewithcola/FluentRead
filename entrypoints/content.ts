...
export default defineContentScript({
  ...
  async main() {
    ...
    browser.runtime.onMessage.addListener(
      (message: { message: string }, sender: any, sendResponse: () => void) => {
...
function setupManualTranslationTriggers() {
  ...
  document.body.addEventListener('mousemove', (event) => {
    ...
    if (
      (config.hotkey === constants.LongPress && Math.abs(event.clientX - startPos.x) > 10) ||
  ...
  document.body.addEventListener('touchstart', (event) => {
    ...
    if (
      ![constants.DoubleClickScreen, constants.TripleClickScreen].includes(config.hotkey) ||
